...
import DeleteWebpage from "../Modals/DeleteWebpage";
...
const WebPageCard = (props) => {
    ...
    return (
        <div className="w-full bg-webPage-card h-min border border-7 border-black rounded-[1.5rem] m-4 text-center py-6 relative">
            ...
            <BiTrash className="absolute top-3 right-3 w-7 h-7 text-red-400 cursor-pointer hover:text-red-500" onClick={() => { setShowDeleteModal(true) }} />
            <img src={props.webpageData?.logo ? props.webpageData?.logo : "./images/awLogo-nobg.png"} className="rounded-full w-[8rem] h-[8rem] mx-auto object-scale-down" />
